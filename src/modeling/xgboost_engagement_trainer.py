...
def load_and_merge() -> pd.DataFrame:
    ...
    clean = pd.read_csv(CLEAN_PATH, usecols=["video_id", "like_rate", "comment_rate"])
    ...
    df = df.merge(clean, on="video_id", how="inner")
    return df
...
def main():
    ...
    for label, mask in [("longform", ~df["is_short"]), ("shorts", df["is_short"])]:
        ...
        save_model(model, f"xgboost_engagement_{label}", best_params)
